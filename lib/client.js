// dsh-unarchive — client bundle (built artifact, ModuleLoader format).
 ...
 *
 * Original UI implementation: sidebar entry (`sidebar.footer.action`),
 * floating panel (`shell.overlay`), inline previews, settings tab, and the
 * optional archive confirmation wrapping `ctx.workspaces.archiveSession`
...
const api = {
  list: () => apiJson('/archived', { headers: { accept: 'application/json' } }),
  preview: (sessionId) => apiJson('/archived/' + encodeURIComponent(sessionId) + '/preview', { headers: { accept: 'application/json' } }),
  restore: (sessionId) => apiJson('/restore', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId }),
  }),
  restoreAll: (ids) => apiJson('/restore-all', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ ids }),
  }),
  title: (sessionId) => apiJson('/title', {
    method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ sessionId }),
  }),
  getSettings: () => apiJson('/settings', { headers: { accept: 'application/json' } }),
  setSettings: (patch) => apiJson('/settings', {
...
var plugin = {
  ...
  apply(ctx) {
    ...
    // ---- shared store (per activation, in-memory; persisted settings live on host) ----
    const listeners = new Set();
    const notify = () => { for (const fn of Array.from(listeners)) { try { fn(); } catch { /* ignore */ } } };
    const store = {
      open: false,
      tab: 'list',
    ...
    // load persisted settings once on startup (server-side file wins over cordis.yml)
    api.getSettings().then((s) => {
      if (s && typeof s === 'object') {
        setStore({
          settings: {
            confirmOnArchive: typeof s.confirmOnArchive === 'boolean' ? s.confirmOnArchive : store.settings.confirmOnArchive,
            showButton: typeof s.showButton === 'boolean' ? s.showButton : store.settings.showButton,
            previewEnabled: typeof s.previewEnabled === 'boolean' ? s.previewEnabled : store.settings.previewEnabled,
          },
          settingsLoaded: true,
        });
      }
    ...
    const setSetting = (key, value) => {
      const next = { ...store.settings, [key]: value };
      setStore({ settings: next });
      api.setSettings({ [key]: value }).catch((e) => warn('settings save failed', e));
    };

    // ---- sidebar entry (native-style footer row: matches the Settings trigger) ----
    function SidebarEntry(props) {
      ...
      return h('button', {
        ...
        h(IconArchiveOutline20, { key: 'i', size: 16 }),
    ...
    function ArchivePanel({ useWorkspaces }) {
      ...
      if (typeof useWorkspaces === 'function') {
        try { reactiveIds = useWorkspaces((st) => (st && st.archivedSessionIds) || []) || []; hasHook = true; } catch { /* ignore */ }
      }
      const archivedIds = reactiveIds;
      ...
      const onTogglePreview = (it) => {
        ...
        api.preview(it.sessionId).then((res) => {
      ...
      const onRestoreAll = () => {
        ...
        setBusyAll(true);
        api.restoreAll(archivedIds).then((res) => {
      ...
      kids.push(h('div', { key: 'head', className: 'dua-panel-head' },
        ...
        h('button', {
          key: 'x', type: 'button', className: 'dua-icon-btn dua-close-btn', autoFocus: true,
          title: t('close'), 'aria-label': t('close'), onClick: () => setStore({ open: false }),
        }, h(IconCloseOutline16, { key: 'ic', size: 14 })),
      ));
      ...
      if (s.tab === 'settings') {
        kids.push(h('div', { key: 'body', className: 'dua-settings' },
          h(Toggle, { key: 'c', checked: s.settings.confirmOnArchive, onChange: (v) => setSetting('confirmOnArchive', v), label: t('settingConfirm'), desc: t('settingConfirmDesc') }),
          h(Toggle, { key: 'b', checked: s.settings.showButton, onChange: (v) => setSetting('showButton', v), label: t('settingButton'), desc: t('settingButtonDesc') }),
          h(Toggle, { key: 'p', checked: s.settings.previewEnabled, onChange: (v) => setSetting('previewEnabled', v), label: t('settingPreview'), desc: t('settingPreviewDesc') }),
    ...
    // ---- styles (theme tokens with fallbacks; own class names; removed on dispose) ----
...
`;
    ...
    if (workspaces && typeof workspaces.archiveSession === 'function') {
      ...
      const next = async (sessionId) => {
        ...
        // fail-open backstop: if the dialog can never be answered, let archiving through
