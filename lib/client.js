...
var plugin = {
  ...
  apply(ctx) {
    ...
    // ---- host bridge (the dynamic build replaces this block with host.call) ----
    const api = {
...
`;
    ...
    ctx.effect(() => {
      ...
      // NOTE: the CSS targets `.dua-root` (class) — a `data-*` attribute alone
      // would leave the capsule in normal document flow (bottom-left, page
      // scroll required). Keep both for debuggability.
      root.className = 'dua-root';
